...
# FIXME: Fix the difference in calculation of distance
max_y_length = Point(min(x), min(y)).distance(Point(min(x), max(y)))
max_x_length = Point(min(x), min(y)).distance(Point(max(x), min(y)))
...
# Calculating the x and y points of the grids, to draw the plot for visualization
# FIXME: CHECK EQUATION - WHEN TO ADD AND WHEN TO SUBTRACT
col_points = list(np.arange(bounds[0], (bounds[2] + square_grid_length), square_grid_length))
row_points = list(np.arange(bounds[1], (bounds[3] + square_grid_length), square_grid_length))
print("column points: ", col_points)
print("row points: ", row_points)

# FIXME: CHECK EQUATION - Fix this calculation
# rows = int(abs(abs(bounds[2]) - abs(bounds[0])) / square_grid_length)
# cols = int(abs(abs(bounds[3]) - abs(bounds[1]+ square_grid_length)) / square_grid_length)
#
